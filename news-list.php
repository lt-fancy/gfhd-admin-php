...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 新闻管理 <span class="c-gray en">&gt;</span> 新闻列表 <a id="refresh" class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
		...
		<table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
			...
			<tbody>
                <?php
                    ...
                    $sql = $mysqli->query("select * from list_info order by is_deleted asc");
                    $datarow = mysqli_num_rows($sql); //长度
                    //循环遍历出数据表中的数据
                    for($i=0;$i<$datarow;$i++){
                        $sql_arr = mysqli_fetch_assoc($sql);
                        $ename = $sql_arr['menu_ename'];
                        $sql1 = $mysqli->query("select * from menu_info where menu_ename='$ename'");
                        ...
                        echo "<tr class='text-c'>
                                ...
                                <td>$cname</td>
                                <td>$title</td>
                                <td><img width='210' class='picture-thumb' src='../../images/$image'></td>
                                <td>$time</td>
                                <td>$isHome</td>",$isOn,$operate,"
                            </tr>";
                    }
                ?>
			</tbody>
